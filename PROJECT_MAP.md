...
## SYSTEM_FLOW
1. **Fetch:** On first load or manual "Refresh News" click, `feedparser` ingests raw RSS feed items (no background scheduler; Streamlit Cloud spins down when idle).
...
5. **Storage:** Only articles with a score >= 6 are stored in the SQLite database (title, link, score, summary ~300 words, image_url).
